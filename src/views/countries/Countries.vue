		...
		<div class="row">
			...
			<div class="col-6 col-sm-4 col-md-3 col-lg-3 my-4 my-sm-1">
				...
				>
					...
					<option v-for="region of regions" :value="region">
						{{ region }}
					</option>
				</select>
			</div>
	...
	>
		...
		>
			...
			<div class="card-body text-start p-4">
				<router-link
					:to="{ name: 'Country', params: { id: country.name.common } }"
					class="nav-link text-dark"
				>
					<h3 class="my-3">{{ country.name.common }}</h3>
				</router-link>
				<p v-show="country.population" class="my-1 country-info">
					Population: <span>{{ country.population.toLocaleString('en') }}</span>
				</p>
				<p v-show="country.region" class="my-0 country-info">
					Region: <span>{{ country.region }}</span>
				</p>
				<p v-show="country.capital.length" class="my-1 country-info">
...
export default {
	...
	setup() {
		...
		watchEffect(() => {
			if (search.value) {
				countriesFiltered.value = countries.value.filter(country =>
